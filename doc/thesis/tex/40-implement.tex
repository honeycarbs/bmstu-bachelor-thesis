...
func (s *SampleService) ConstructObservationSequence(sm entity.Sample) []int {
	...
	for i := 0; i < len(observations); i++ {
		observations[i] = sm.Frames[i].ClusterIndex
	}
	return observations
}
\end{lstlisting}
...
func (hmm *HiddenMarkovModel) BaumWelch(obs []int, iterations int) {
	var (
		alpha = make([][]float64, len(obs))
		beta  = make([][]float64, len(obs))
		gamma = make([][]float64, len(obs))
		xi    = make([][][]float64, len(obs)-1)
	)
	for i := 0; i < len(obs); i++ {
	...
	for it := 0; it < iterations; it++ {
		hmm.Forward(obs, alpha)
		hmm.Backward(obs, beta)
		hmm.computeGammaProbs(obs, alpha, beta, gamma)
		hmm.computeXiProbs(obs, alpha, beta, xi)
		
		hmm.update(obs, gamma, xi)
	}
	hmm.laplaceSmoothing(obs)
}
\end{lstlisting}
...
Вероятности $\alpha$, $\beta$ и $\gamma$ используются при вычислении $\xi$-вероятностей. Вычисление $\xi$-вероятностей представлено на листинге  \ref{lst:ml:xi}.
\begin{lstlisting}[
...
\begin{table}[H]
	...
	\begin{tabular}{|O|T|P|T|}
		...
		1 & последовательность наблюдаемых состояний содержит различные состояния & эмиссии, наблюдаемые состояния & алгоритм правильно откорректирует матрицу эмиссий \\ \hline
		2 & последовательность наблюдаемых состояний содержит одно и то же состояние & эмиссии, наблюдаемые состояния & алгоритм правильно откорректирует матрицу эмиссий \\ \hline
	\end{tabular}
\end{table}
...
Вспомогательная функция, запускающая алгоритм алгоритм Баума~--- ~Велша для тестов представлена на листинге \ref{lst:test:bw}.
\begin{lstlisting}[
